...
RepoId LegacyDiscovery::bit_key_to_repo_id(const DomainParticipantImpl* participant,
...
// TODO(jrw972): The should not take a DomainParticipantImpl at all or
// at least it should be const.  Currently, this is needed to 1) call
// set_id on the domain participant and 2) allow RTPS discovery to
// initialize security related members.  This can be fixed once we
// strip discovery identifiers and move security related members into
// the core or all the way into RtpsDiscovery.
DDS::ReturnCode_t LegacyDiscovery::add_domain_participant(const DDS::DomainId_t domain_id,
                                                          DomainParticipantImpl* dp) {
  return discovery_->add_domain_participant(domain_id, dp);
}
