                                   ...
                                   ACE_Reactor* reactor)
{
  ...
  if (!reactor_wrapper_.open(reactor_) && log_level >= LogLevel::Error) {
                                                 ...
                                                 const TimeDuration& interval)
{
  ...
  const TimerId tid = timer_heap_.schedule(&handler, arg, delay.value(), interval.value());
...
void ReactorWrapper::arm()
{
  static const timespec one_ns = {0, 1};
  const ACE_Time_Value delay = MonotonicTimePoint::now().value() - timer_heap_.earliest_time();
  itimerspec ts;
